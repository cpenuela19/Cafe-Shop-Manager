...
public class IngredientService {
    ...
    public IngredientEntity createIngredient(IngredientEntity ingredientEntity) {
        ...
        if (normalizedName.length() < 3)
            throw new IllegalOperationException("Ingredient name must be at least 3 characters long");
    ...
    // Cannot delete an ingredient that has price history records
    @Transactional
    public void deleteIngredient(IngredientEntity ingredientEntity) {
        if (priceHistoryRepository.existsByIngredientId(ingredientEntity.getId()))
            throw new IllegalOperationException("Cannot delete ingredient '" + ingredientEntity.getName() +
                    "': it has price history records. Deactivate it instead.");
        ingredientRepository.delete(ingredientEntity);
    }

    // Cannot reactivate an ingredient that has no active supplier
    @Transactional
    public IngredientEntity reactivateIngredient(IngredientEntity ingredientEntity) {
        if (ingredientEntity.isActive())
            throw new IllegalOperationException("Ingredient is already active");
        boolean hasActiveSupplier = ingredientEntity.getSupplierIngredient().stream()
                .anyMatch(si -> si.getSupplier().isActive());
        if (!hasActiveSupplier)
            throw new IllegalOperationException("Cannot reactivate ingredient '" + ingredientEntity.getName() +
                    "': it has no active supplier");
